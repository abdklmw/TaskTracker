	...
	public class TimeEntriesController : Controller
	{
			...
			bool invoiceSentDateAny = false)
		{
			...
			try
			{
				...
				_logger.LogInformation("Dynamic TimezoneOffset for TimeEntries Index: {TimezoneOffset} minutes, DST Active: {IsDst}, TimeZoneId: {TimeZoneId}", timezoneOffset, userTimeZone.IsDaylightSavingTime(nowUtc), user.TimeZoneId);
			}
			catch (TimeZoneNotFoundException ex)
			{
				_logger.LogError("Invalid TimeZoneId {TimeZoneId} for user {UserId}: {Error}", user.TimeZoneId, userId, ex.Message);
				timezoneOffset = 0;
			}
			...
			_logger.LogInformation("Index called with page={Page}, recordLimit={RecordLimit}, clientFilter={ClientFilter}, projectFilter={ProjectFilter}, invoicedDateStart={InvoicedDateStart}, invoicedDateEnd={InvoicedDateEnd}, paidDateStart={PaidDateStart}, paidDateEnd={PaidDateEnd}, invoiceSentDateStart={InvoiceSentDateStart}, invoiceSentDateEnd={InvoiceSentDateEnd}, invoicedDateAny={InvoicedDateAny}, paidDateAny={PaidDateAny}, invoiceSentDateAny={InvoiceSentDateAny}, hasFilters={HasFilters}",
				...
				invoicedDateAny, paidDateAny, invoiceSentDateAny, totalRecords > 0);
		...
		public async Task<IActionResult> Create([Bind("ClientID,ProjectID,Description,StartDateTime,EndDateTime,HoursSpent")] TimeEntry timeEntry, string? action)
		{
			...
			try
			{
				...
				var nowUtc = DateTimeOffset.UtcNow.UtcDateTime;
				timezoneOffset = (int)userTimeZone.GetUtcOffset(nowUtc).TotalMinutes;
				_logger.LogInformation("Dynamic TimezoneOffset for Create: {TimezoneOffset} minutes, TimeZoneId: {TimeZoneId}", timezoneOffset, user.TimeZoneId);
			}
			catch (TimeZoneNotFoundException ex)
			{
				_logger.LogError("Invalid TimeZoneId {TimeZoneId} for user {UserId}: {Error}", user.TimeZoneId, userId, ex.Message);
			...
			if (action == "StartTimer")
			{
				...
				if (ModelState.IsValid)
				{
					...
					if (success)
					{
						_logger.LogInformation("Timer started for user {UserId}, TimeEntryID: {TimeEntryID}, StartDateTime: {StartDateTime}", userId, timeEntry.TimeEntryID, timeEntry.StartDateTime);
			...
			else
			{
				if (ModelState.IsValid)
				{
					...
					if (success)
					{
						_logger.LogInformation("Time entry created for user {UserId}, TimeEntryID: {TimeEntryID}", userId, timeEntry.TimeEntryID);
		...
		public async Task<IActionResult> StopTimer(int TimeEntryID)
		{
			...
			if (success)
			{
				_logger.LogInformation("Timer stopped for user {UserId}, TimeEntryID: {TimeEntryID}", userId, TimeEntryID);
		...
		public async Task<IActionResult> Edit(int id, [Bind("TimeEntryID,UserId,ClientID,ProjectID,Description,StartDateTime,EndDateTime,HoursSpent")] TimeEntry timeEntry)
		{
			...
			if (ModelState.IsValid)
			{
				...
				if (success)
				{
					_logger.LogInformation("Time entry updated for user {UserId}, TimeEntryID: {TimeEntryID}", userId, timeEntry.TimeEntryID);
		...
		public async Task<IActionResult> DeleteConfirmed(int id)
		{
			...
			if (success)
			{
				_logger.LogInformation("Time entry deleted for user {UserId}, TimeEntryID: {TimeEntryID}", userId, id);
